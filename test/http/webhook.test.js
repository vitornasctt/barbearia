...
test.beforeEach(async () => {
  ...
  config.WHATSAPP_APP_SECRET = SEGREDO;
});
test.afterEach(() => { config.WHATSAPP_APP_SECRET = ''; });
...
test('POST /webhooks/whatsapp: resposta SIM confirma o agendamento pendente do celular', async () => {
  ...
  const res = await request(buildApp()).post('/webhooks/whatsapp')
    .set('content-type', 'application/json').set('x-hub-signature-256', assinar(body)).send(body);
  assert.equal(res.status, 200);
  const row = await query(`SELECT status FROM agendamentos WHERE id=$1`, [ag]);
  assert.equal(row.rows[0].status, 'confirmado');
});

test('POST /webhooks/whatsapp: assinatura errada => 403', async () => {
  const payload = {
    entry: [{ changes: [{ value: { messages: [{ from: '5528999990000', text: { body: 'SIM' } }] } }] }],
  };
  const body = JSON.stringify(payload);
  const res = await request(buildApp()).post('/webhooks/whatsapp')
    ...
    .set('x-hub-signature-256', 'sha256=' + '0'.repeat(64)).send(body);
  assert.equal(res.status, 403);
});

test('sem WHATSAPP_APP_SECRET configurado: GET e POST => 404', async () => {
  config.WHATSAPP_APP_SECRET = '';
  const g = await request(buildApp()).get('/webhooks/whatsapp')
    .query({ 'hub.mode': 'subscribe', 'hub.verify_token': 'x', 'hub.challenge': '123' });
  assert.equal(g.status, 404);
  const p = await request(buildApp()).post('/webhooks/whatsapp')
    .set('content-type', 'application/json').send(JSON.stringify({ entry: [] }));
